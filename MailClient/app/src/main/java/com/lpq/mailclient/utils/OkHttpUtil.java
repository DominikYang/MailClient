package com.lpq.mailclient.utils;/**
 * Created by WeiYuyang on 2019/11/24.
 */

import android.util.Log;
import com.lpq.mailclient.Exception.GlobalException;
...
public class OkHttpUtil {
    ...
    public Response postData(String url, Map<String, String> bodyParams,String token) {
        ...
        //2 构造Request
        Request.Builder requestBuilder = new Request.Builder();
        Request request = requestBuilder.post(body).header("token",token).url(url).build();
        //3 将Request封装为Call
        Call call = mOkHttpClient.newCall(request);
        //4 执行Call，得到response
        Response response = null;
    ...
    public String postJson(String url, String json,String token) throws IOException {
        RequestBody body = RequestBody.create(JSON, json);
        Request request = new Request.Builder()
                .url(url)
                .post(body)
